...
-- 1. Add columns to public.enseignants
ALTER TABLE public.enseignants ADD COLUMN IF NOT EXISTS type_contrat TEXT DEFAULT 'CDI';
ALTER TABLE public.enseignants ADD COLUMN IF NOT EXISTS categorie TEXT DEFAULT 'Enseignant';

-- 2. Update trigger sync_enseignant_to_personnel
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
    INSERT INTO public.membres_personnel (
      ...
      NEW.id, NEW.nom, NEW.prenom, ens_email, NEW.telephone, NEW.sexe, COALESCE(NEW.categorie, 'Enseignant'), COALESCE(NEW.type_contrat, 'CDI'), COALESCE(NEW.salaire_mensuel, 0), COALESCE(NEW.date_embauche, CURRENT_DATE), COALESCE(NEW.statut, 'actif'), NEW.etablissement_id, NEW.created_at
...
-- 3. Update trigger sync_personnel_to_enseignant
  ...
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
    IF (NEW.categorie = 'Enseignant') THEN
      INSERT INTO public.enseignants (
        id, nom, prenom, email, telephone, sexe, matiere_principale, salaire_mensuel, date_embauche, statut, type_contrat, categorie, etablissement_id, created_at
      ) VALUES (
        NEW.id, NEW.nom, NEW.prenom, NEW.email, NEW.telephone, NEW.sexe, 'Général', COALESCE(NEW.salaire_de_base, 0), COALESCE(NEW.date_embauche, CURRENT_DATE), COALESCE(NEW.statut, 'actif'), COALESCE(NEW.type_contrat, 'CDI'), COALESCE(NEW.categorie, 'Enseignant'), NEW.etablissement_id, NEW.created_at
      ) ON CONFLICT (id) DO UPDATE SET
        nom = EXCLUDED.nom,
        prenom = EXCLUDED.prenom,
        email = EXCLUDED.email,
