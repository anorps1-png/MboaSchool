...
async function run() {
  ...
  try {
    ...
    const securityRes = await client.query(`
      ...
      WHERE n.nspname = 'public' AND c.relname = 'enseignants'
    `);
    console.log("\n--- Row Security on enseignants ---");
    console.table(securityRes.rows);

    // Check all policies for public.enseignants
    const policiesRes = await client.query(`
      SELECT policyname, roles, cmd, qual, with_check 
      FROM pg_policies 
      WHERE tablename = 'enseignants'
    `);
    console.log("\n--- Policies on enseignants ---");
