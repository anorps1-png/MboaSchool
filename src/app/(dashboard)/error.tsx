...
export default function DashboardError({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <div className="w-14 h-14 rounded-full bg-red-bg flex items-center justify-center text-2xl mb-4">
        ⚠️
      </div>
      <h2 className="text-xl font-bold text-ink">Cette page a rencontré un problème</h2>
      <p className="text-sm text-ink-soft mt-2 max-w-md">
        L&apos;incident a été enregistré. Vous pouvez réessayer ou revenir plus tard.
      </p>
      <button
        onClick={() => reset()}
        className="mt-6 px-5 py-2 bg-accent hover:bg-accent-hover text-cream rounded-control text-sm font-bold transition-colors"
      >
        Réessayer
      </button>
    </div>
